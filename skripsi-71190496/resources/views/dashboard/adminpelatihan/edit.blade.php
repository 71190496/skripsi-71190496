    ...
    <div class="col-lg-8 mb-4 ">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <form method="post" action="/dashboard/daftarpelatihan/{{ $data->id }}" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add-field').click(function() {
                var field =
                    '<div id="fasilitator-pelatihan" class="form-group mt-2 mb-2"><label for="fasilitator-pelatihan">Fasilitator Pelatihan</label><select name="fasilitator[]" class="form-control">@foreach ($fasilitator_pelatihan as $item)<option value="{{ $item->id_fasilitator }}">{{ $item->nama_fasilitator }}</option>@endforeach</select></div>';
            ...
            $('#fasilitator-pelatihan').on('#simpan', function(e) {
                e.preventDefault();
                ...
                //         url: '/save-data',
                //         type: 'POST',
                //         data: {keyname:$('#selected-value').val()},
                ...
                $.ajax({
                    url: '/save-data',
                    type: 'POST',
                    data: $('#selected-value').filter(":selected").val(),
                    success: function(response) {
                        console.log(response);
                    },
                    error: function(xhr, status, error) {

...
@section('container')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form id="hidden-form" action="{{ route('dashboard.evaluasi.updatekonsultasi', ['id' => $id]) }}" method="post"
                style="display: none;">
                @csrf
            ...
            <div id="form-builder-container" style="overflow: auto; height: 600px; width: 100%">
    ...
    <script>
        jQuery(function($) {
            ...
            formBuilderPromise.then(function(formBuilder) {
                $.ajax({
                    url: "{{ route('dashboard.evaluasi.editkonsultasi', ['id' => $id]) }}",
                    type: 'GET',
                    success: function(data) {
                        if (data && data.content) {
                            console.log('Content:', data.content);

                            // Memeriksa jenis data.content
                            console.log(typeof data.content);

                            // Mengonversi string JSON menjadi objek JavaScript jika perlu
                            var formData = Array.isArray(data.content) ? data.content : JSON
                                .parse(data.content);
